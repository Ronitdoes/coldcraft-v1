...
export default function Dashboard() {
  ...
  const totalMails = mailHistory.length;
  const uniqueCompanies = new Set(mailHistory.map(m => m.company.toLowerCase())).size;
  const lastActiveDate = mailHistory.length > 0 
    ? new Date(mailHistory[0].created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase()
    : "NEVER";
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-black text-white pb-24">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-black border-b border-white/5 px-8 pt-8 pb-4 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity z-20">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {profile.skills.map((skill, i) => (
                            <span key={i} className="border border-white/5 font-mono text-[8px] uppercase tracking-[0.2em] px-2 py-0.5 text-white/30">
                              {skill}
                            </span>
                          ))}
                        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-12 pt-32 md:pt-40">
        ...
        <div className="anim-cta mb-12 flex flex-wrap gap-4 md:gap-6">
          <PrimaryButton 
            ...
            className="w-full md:w-auto !rounded-none !min-w-0 px-10 py-5 shadow-none hover:shadow-none"
          />
          <PrimaryButton 
            title="Follow Up Mail"
            subtitle="Stay Persistent"
            onClick={() => router.push("/compose?type=followup")}
            className="w-full md:w-auto !rounded-none !min-w-0 px-10 py-5 shadow-none hover:shadow-none border border-white/10"
          />
        </div>
                    ...
                    <div key={mail.id} className="border border-white/5 flex flex-col hover:border-white/20 transition-colors duration-200">
                      ...
                      <div className="px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <Link 
                            href={`/compose?company=${encodeURIComponent(mail.company)}&role=${encodeURIComponent(mail.role)}&tone=${encodeURIComponent(mail.tone)}`}
                            className="font-mono text-[10px] uppercase tracking-[0.15em] text-white/30 hover:text-white transition-colors"
                          >
                            RESEND
                          </Link>
